...
##### Latent Dirichlet Allocation（无监督文本数据主题建模模型）
* 选择原因：
1.高效主题挖掘：LDA 能够高效从大量文档中提取文档主题，从而为文本数据提供高层次的理解，帮助用户快速识别信息的主题分布。。
2.无监督学习：LDA 不需要标注数据即可进行主题建模，适合各种未标记文档集。
3.灵活性：LDA 模型适用于多种类型的文本数据，包括社交媒体帖子、新闻文章和研究论文等，应用广泛。
* 具体方法：
1.LDA 假设每个文档是多个潜在主题的混合，而每个主题又由多个单词组成，形成概率分布。
...
|Comment_tokenizer|调用评论分词接口|景区名称|评论分词分析结果|无|
